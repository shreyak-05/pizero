...
class URTDEControllerConfig:
    ...
    ## --------- Newly Added ----------- ##
    agent: str = "ur"
    hostname: str = "192.168.77.134"
    # hostname: str = "192.168.77.243"
    robot_port: int = 50002  # for trajectory
    robot_ip: str = "192.168.77.22" 
...
class URTDEController:
    ...
    def __init__(self, cfg: URTDEControllerConfig, task) -> None:
        ...
        self.y_min, self.y_max = [-0.453, -0.11]
        ...
        if len(positions) == len(curr_pose):
            ...
            for angle in rpy_delta:
                if abs(angle) > 0.3:
                    ...
                    print("Angle difference is too large")
                    # temp override
            ...
            # if positions[0] < self.x_min:
            #     positions[0] = self.x_min
            # if positions[1] < self.y_min:
            #     positions[1] = self.y_min
            # if positions[2] < self.z_min: 
            #     positions[2] = self.z_min
                
            # if positions[0] > self.x_max:
            #     positions[0] = self.x_max
            # if positions[1] > self.y_max:
            #     positions[1] = self.y_max
            # if positions[2] > self.z_max:
            #     positions[2] = self.z_max
    ...
    def update(self, action: list[float]) -> None:
        ...
        if self._robot.get_joint_state()[1] < -2.7:
            return
